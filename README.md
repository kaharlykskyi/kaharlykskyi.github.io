...
#### 3.1. GitHub Pages
Включіть [GitHub Pages](https://pages.github.com/), будь ласка. Не обов'язково, але це значно полегшить процес перевірки домашки, адже мені не доведеться скачувати і відкривати файл! Йееее. Ну, це не є непосильним завданням, звичайно, але й включити GitHub Pages і винести **index.html** в папку **/docs** теж не вимагає надмірних зусиль. Та там воно навіть і в гілці `master` може лежати.

#### 3.2. freeCodeCamp
Базові уроки по JavaScript є на популярному порталі [**freeCodeCamp**](https://www.freecodecamp.org/). Там є окремий розділ для початківців, в якому пояснюються прямо таки основи (розділ розрахований на 10 годин). Ну але це вже якщо все аж дуже незрозуміло. Я очікую, що людям з бекграундом в PHP і .NET JavaScript буде інтуїтивно зрозумілим і проблеми виникнуть лише з синтаксисом, який треба буде час від часу підглядати.
